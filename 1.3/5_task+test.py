...
# Итак, задача простая, необходимо отсортировать список зарплат и список сотрудников.
# В отсортированных списках индексы соответствуют. Но нельзя изменять список сотрудников.
# Но можно сделать копию списка сотрудников, отсортировать в кортеж, присвоить дополнительный индекс, по которому соотнести индексы копии списка сотрудников и зарплаты.
# Затем присвоить ранк индекса копии списка сотрудников с ранком индекса таблицы зарплат и вывести таблицу зарплат в соответствии с рангом оригиального списка сотрудников.
...
#           result.append(0)
#      for i in range(N):
#           y = sorted_index_ids[i][1]
#           result[y] = sorted_salary[i] # тут была ошибка, сделал sorted_salary[y], y - исходный индекс
#      return result       
...
def SynchronizingTables(N, ids, salary):
     ...
     for i in range(N):
          ...
          result[start_index] = sorted_salary[i] # тут была ошибка, сделал sorted_salary[y], y - исходный индекс
     return result 
print(SynchronizingTables(3, [50, 1, 1024], [20000, 100000, 90000]))