...
class SimpleTree:
    ...
    def EvenTrees(self):
        result = []
        self.EvenFunc(self.Root, result)
        return result
    def CountTrees(self,node):
        count = 1 
        for child in node.Children:
            count += self.CountTrees(child)
        return count
    def EvenFunc(self,node, result):
        for child in node.Children:
            self.EvenFunc(child, result)
            if self.CountTrees(child) % 2 == 0:
                result.append(node)
                result.append(child)
